...
## What I learned
- This project was my first Unity project. I started it with the intention of making a two-week long basic pirate ship game, in which a pirate ship travels around shooting cannonballs at sea monsters and plundering the seas. However, my ambitions quickly grew out of proportion. I wanted to try and tackle a gripe I had with other top-down pixel art water: faking it. I wanted water that dynamically changed and could exert forces on the player. As you'll soon discover, this went through a couple iterations.
...
- Due to the amount of control I wanted over the water's appearance, there are a ridiculous amount of parameters to each function. This is a great example of how I could have improved code. There are two specific things I've learned that could clean this up drastically--scriptable objects, and class instancing.
  ...
  To finish the buoyancy script, the heights of the ship's buoys and the water's buoys were compared, and based on their difference a force was applied. Similarly, the heights of the ship's buoys relative to each other was compared to rotate the ship. Now that forces were dynamically added, depending on where the ship steered it's speed, top speed, and acceleration could be appropriately modified.
...
- The next system that was added was a splashing system. An advantage of Gerstner waves was the ease with which I could further displace vertices; I only needed to add the splash function to the pre-existing Gerstner function. To do this, I created a custom node in the HLSL script that could create a splash and used generic wave propogation equations straight out of my mechanics class. 
  ...
  void Splash_float(float3 worldPos, float waveLength, float fadeSpeed, float fallOffDist, float timePassed, out float3 displacement)
  {
    ...
    for (int i = 0; i < _WaveCount; i++)
    {
        ...
        if (dist < maxRadius)
        {
            ...
            float fadeOut = saturate(fallOffDist / (dist + 1) * fallOffDist / (dist + 1));
			
            float fadeIn = saturate((timePassed - _TimePassed[i]) * fadeSpeed);
			
            float die = saturate((timePassed - _TimePassed[i]) / _LifeSpans[i]);
			
            y_value += lerp(basic * fadeOut * fadeIn, 0, die);
            //y_value += basic * fadeOut * fadeIn;
        }
    }
  ...
  To transfer data from script to HLSL, I designed a class named SplashInfo. It currently has three functions: AddWave, UpdateShader, and DeleteWaves. What's nice is that SplashInfo contains modifiable lists, but when it transfers data to the water material, it only sends an array (currently of size 16). There is one instance of SplashInfo that exists, so other scripts can easily access it and summon splashes. Currently, the camera performs a raycast when the user left-clicks. If the raycast hits water, then a splash is spawned at that location. The boat also constantly cycles through splashes to create a wake. Finally, in the same MonoBehavior as SplashInfo (but outside the class!) there is a function that can change the water intensity. It is a multiplier to the Gerstner waves' amplitudes. This can be used for dynamic weather. 
